...
def determine_differencing_order(data, max_d=3):
    """
    @brief Determine optimal differencing order using ADF test
    @details Uses Augmented Dickey-Fuller test to find minimum differencing needed for stationarity
    @param data Time series data to test
    @param max_d Maximum differencing order to test
    @return Optimal differencing order (d parameter)
    @note Essential for ARIMA model parameter selection
    """
    ...
    for d in range(1, max_d + 1):
        try:
            ...
            if len(diff_data) > 10:  # Ensure enough data points
...
def evaluate_arima_model(data, order):
    """
    @brief Evaluate a single ARIMA model configuration using AIC
    @details Fits ARIMA model with given parameters and returns AIC score
    @param data Time series data for fitting
    @param order Tuple of (p, d, q) parameters
    @return AIC score (lower is better) or large number if fitting fails
    @note Used in grid search to find optimal parameters
    """
    try:
...
def grid_search_arima(data, max_p=5, max_q=5, max_d=2):
    """
    @brief Perform grid search to find optimal ARIMA parameters
    @details Tests all combinations of p, d, q parameters within specified ranges
    @param data Time series data for fitting
    @param max_p Maximum AR order to test
    @param max_q Maximum MA order to test  
    @param max_d Maximum differencing order to test
    @return Tuple of best (p, d, q) parameters
    @note Replacement for pmdarima's auto_arima functionality
    """
    if data is None or len(data) == 0:
    ...
    # First determine optimal d using statistical test
    optimal_d = determine_differencing_order(data, max_d)
    
    # Grid search over p and q, using determined d
    ...
    if best_order is None:
        print("No suitable ARIMA model found, using default parameters")
        best_order = CONFIG['DEFAULT_ARIMA_ORDER']
...
def train_auto_arima(train_data_diff, hyperparams=None):
    """
    @brief Train ARIMA model with automatic parameter selection using grid search
    @details Replaces pmdarima's auto_arima with statsmodels + custom grid search
    ...
    @note Uses grid search instead of pmdarima for parameter optimization
    """
    ...
    try:
        # Set default parameters for grid search
        default_params = {
            'max_p': min(CONFIG['DEFAULT_MAX_P'], 5),  # Limit for reasonable runtime
            'max_q': min(CONFIG['DEFAULT_MAX_Q'], 5),
            'max_d': min(CONFIG['DEFAULT_MAX_D'], 2)
        }
        
        # Update parameters with user-provided hyperparameters
        if hyperparams:
            # Map pmdarima-style parameters to our grid search parameters
            if 'max_p' in hyperparams:
                default_params['max_p'] = hyperparams['max_p']
            if 'max_q' in hyperparams:
                default_params['max_q'] = hyperparams['max_q']
            if 'max_d' in hyperparams:
                default_params['max_d'] = hyperparams['max_d']
        
        # Find optimal parameters using grid search
        best_order = grid_search_arima(train_data_diff, **default_params)
        
        # Fit the best model
        print(f"Training final ARIMA model with order {best_order}")
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore')
            final_model = ARIMA(train_data_diff, order=best_order)
